...
import Waring from '@/components/icons/Waring';
import WarningWrap from '@/components/icons/WarningWrap';
import FillHeart from '@components/icons/FilledHeart';
import { IProduct } from '@remote/api/types/home';
import Text from '@shared/text/Text';

import styles from './FixProductArticle.module.scss';

interface FixProductArticleProps {
  isRow?: boolean;
...
function FixProductArticle({
  isRow = false,
  itemData,
}: FixProductArticleProps) {
      ...
      <article className={cx('container', { row: isRow })}>
        <div className={cx('imgBox')}>
          <Image
            ...
            src={
              itemData.item === '코팅제'
                ? TYPE.코팅제[getRandomNumber()]
                : TYPE.세정제[getRandomNumber()]
            }
            alt="제품 이미지"
            width={0}
            height={0}
...
export default FixProductArticle;
